...
export default function Relatorios() {
  ...
  // Função para converter SVG em PNG (para PDF)
  ...
  const gerarPDF = async () => {
    ...
    // Cards
    const cards = [
      { title: "Total de Ordens", value: dados.totalOrdens },
      { title: "Concluídas", value: dados.concluidas },
      { title: "Pendentes", value: dados.pendentes },
      { title: "Total Recebido", value: `R$ ${Number(dados.totalRecebido).toFixed(2)}` },
      { title: "A Receber", value: `R$ ${Number(dados.totalReceber).toFixed(2)}` },
    ];
    ...
    cards.forEach((card) => {
      doc.text(card.title, 40, y);
      doc.text(String(card.value), pageWidth - 40, y, { align: "right" });
      y += 20;
    });
